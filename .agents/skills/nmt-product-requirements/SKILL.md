...
> **Producer contract (binding) — `../PRODUCER-CONTRACT.md`.** Six cross-cutting behaviors shared by all producer skills, from user feedback: (1) print a **helicopter-view** before the first question; (2) ask **Markdown or HTML** output; (3) treat **all** user input as hypothesis and emit a *"risks I see in what you gave me"* block; (4) print **validation debt** and write **`GO (to validation)`**, never bare `GO`; (5) accept a **custom output path**; (6) Deep mode runs an **evidence floor + self-critic loop** and uses an **internet-access ladder + Evidence Pack fallback**. The hooks below wire each into this skill; the contract is the source of truth for the wording. This skill is the **closest-to-build** artifact in the chain, so the validation-debt + *"validate before you build, don't build yet"* framing (§4) carries the most weight here — get it right.
...
> **Two modes:** *Quick* (default — no internet, ~5–10 min, reasoning only; good for a first spec from what you already know) · *Deep* (opt-in — subagents + web research, longer; refreshes competitor parity and stress-tests edge cases against real reviews; uses direct Codex web access).
...
- **Evidence floor (not just the cap).** Treat each web leg's lower bound as a *floor*, not only a ceiling: the PARITY leg and the EDGE review-mining leg may not return "done" until each has hit a real minimum of distinct sources for its task. If it cannot reach that minimum after the retry loop, it must stop and request an Evidence Pack per `PRODUCER-CONTRACT.md §6`. "Made two queries and stopped" is a failure state, not a completion — the leg re-runs.
...
- **Internet-access ladder + Evidence Pack fallback.** Follow `PRODUCER-CONTRACT.md §6`: native Codex web/search/open → retry/self-critic → mandatory read-only shell HTTP attempt. Native blocking, authorization failure, thin results, or a source-specific 401/403 do not prove internet access is unavailable. Request an Evidence Pack only if the floor remains unmet after the full ladder; never bypass access controls, use non-read-only HTTP methods, recommend MCP/Firecrawl/Exa, invent facts, proceed thin, or label the output Deep. If the user declines an Evidence Pack, offer an explicit switch to Quick.
---
